...
if (isset($_POST['email'])) 
{
  ...
  {
	$checkmail = "SELECT * from userr WHERE email = '$email'";
	$runcheck = mysqli_query($conn , $checkmail) or die(mysqli_error($conn));
	if (mysqli_num_rows($runcheck) > 0) 
	{
		$played_on = date('d-m-y  H:i:s');
		$update = "UPDATE userr SET played_on = '$played_on' WHERE email = '$email' ";
	...
	$query = "INSERT INTO userr(email,played_on) VALUES ('$email','$played_on')";
	$run = mysqli_query($conn, $query) or die(mysqli_error($conn)) ;
	
	if (mysqli_affected_rows($conn) > 0) 
	   {
		$query2 = "SELECT * FROM userr WHERE email = '$email' ";
